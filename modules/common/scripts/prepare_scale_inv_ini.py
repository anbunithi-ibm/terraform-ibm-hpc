...
def calculate_maxFilesToCache(nodeclass, memory):
    ...
    if nodeclass == "computenodegrp":
        ...
        if calFilesToCache < 1024:
            ...
            maxFilesToCache = str(int(min((calFilesToCache / 1024), 3)) // 1) + "M"
...
def generate_nodeclass_config(nodeclass, memory, vcpus, bandwidth):
    ...
    cluster_tuneable_details = [
        {"nodeclass_name": check_nodeclass_name},
        {
...
def initialize_cluster_details(
    ...
    compute_subnet_cidr,
    protocol_gateway_ip,
    scale_remote_cluster_clustername,
    ...
    cluster_details["scale_remote_cluster_clustername"] = (
        scale_remote_cluster_clustername
    )
    # Preparing list for Encryption Servers
    if scale_encryption_servers:
        cleaned_ip_string = (
            scale_encryption_servers.strip("[]").replace('\\"', "").split(",")
        )
...
def initialize_node_details(
    ...
    storage_nsd_server_instance_names,
    ...
    if cls_type == "compute":
        ...
        for each_ip in storage_cluster_instance_names:
            if storage_cluster_instance_names.index(each_ip) <= (
                start_quorum_assign
            ) and storage_cluster_instance_names.index(each_ip) <= (manager_count - 1):
            ...
            elif storage_cluster_instance_names.index(each_ip) <= (
                start_quorum_assign
            ) and storage_cluster_instance_names.index(each_ip) > (manager_count - 1):
                node = {
                    "ip_addr": each_ip,
        ...
        if az_count > 1:
            if len(storage_private_ips) - len(desc_private_ips) >= quorum_count:
                ...
                quorums_left = (
                    quorum_count - len(storage_private_ips) - len(desc_private_ips)
                )
        else:
...
def initialize_scale_config_details(list_nodclass_param_dict):
    ...
    for param_dicts in list_nodclass_param_dict:
        if param_dicts[1] != {}:
            scale_config["scale_config"].append(
                {
                    "nodeclass": list(param_dicts[0].values())[0],
                    "params": [param_dicts[1]],
                }
            )
...
def get_disks_list(az_count, disk_mapping, desc_disk_mapping, disk_type):
    ...
    # Map storage nodes to failure groups based on AZ and subnet variations
        ...
        if az_count == 1:
            ...
            failure_group1 = [
                key for index, key in enumerate(disk_mapping) if index % 2 == 0
            ]
            failure_group2 = [
                key for index, key in enumerate(disk_mapping) if index % 2 != 0
            ]
...
def initialize_scale_storage_details(
    az_count,
    fs_mount,
    block_size,
    disk_details,
    default_metadata_replicas,
    max_metadata_replicas,
    ...
    """
    filesets_name_size = {key.split("/")[-1]: value for key, value in filesets.items()}
...
def initialize_scale_ces_details(
    smb,
    nfs,
    object,
    export_ip_pool,
...
if __name__ == "__main__":
    ...
    PARSER.add_argument(
        "--max_pagepool_gb", help="maximum pagepool size in GB", default=1
    )
    PARSER.add_argument("--disk_type", help="Disk type")
    PARSER.add_argument(
        "--default_data_replicas", help="Value for default data replica"
    )
    PARSER.add_argument("--max_data_replicas", help="Value for max data replica")
    PARSER.add_argument(
        "--default_metadata_replicas", help="Value for default metadata replica"
    )
    PARSER.add_argument(
        "--max_metadata_replicas", help="Value for max metadata replica"
    )
    ...
    PARSER.add_argument(
        "--gui_username", required=True, help="Spectrum Scale GUI username"
    )
    PARSER.add_argument(
        "--gui_password", required=True, help="Spectrum Scale GUI password"
    )
    PARSER.add_argument(
        "--enable_mrot_conf", required=True, help="Configure MROT and Logical Subnet"
    )
    PARSER.add_argument(
        "--enable_ces", required=True, help="Configure CES on protocol nodes"
    )
    PARSER.add_argument("--verbose", action="store_true", help="print log messages")
    PARSER.add_argument(
        "--scale_encryption_servers",
        help="List of key servers for encryption",
        default=[],
    )
    PARSER.add_argument(
        "--scale_encryption_admin_password",
        help="Admin Password for the Key server",
        default="null",
    )
    PARSER.add_argument(
        "--scale_encryption_type",
    ...
    PARSER.add_argument(
        "--scale_encryption_enabled", help="Enabling encryption feature", default=False
    )
    ...
    PARSER.add_argument(
        ...
        help="It checks if protocol node count is less than storage NSD node count",
        default=False,
    )
    PARSER.add_argument("--comp_memory", help="Compute node memory", default=32)
    PARSER.add_argument(
        "--comp_vcpus_count", help="Compute node vcpus count", default=8
    )
    PARSER.add_argument(
        "--comp_bandwidth", help="Compute node bandwidth", default=16000
    )
    PARSER.add_argument("--mgmt_memory", help="Management node memory", default=32)
    PARSER.add_argument(
        "--mgmt_vcpus_count", help="Management node vcpus count", default=8
    )
    PARSER.add_argument(
        "--mgmt_bandwidth", help="Management node bandwidth", default=16000
    )
    PARSER.add_argument(
        "--strg_desc_memory", help="Tie breaker node memory", default=32
    )
    PARSER.add_argument(
        "--strg_desc_vcpus_count", help="Tie breaker node vcpus count", default=8
    )
    PARSER.add_argument(
        "--strg_desc_bandwidth", help="Tie breaker node bandwidth", default=16000
    )
    PARSER.add_argument("--strg_memory", help="Storage NDS node memory", default=32)
    PARSER.add_argument(
        "--strg_vcpus_count", help="Storage NDS node vcpuscount", default=8
    )
    PARSER.add_argument(
        "--strg_bandwidth", help="Storage NDS node bandwidth", default=16000
    )
    PARSER.add_argument("--proto_memory", help="Protocol node memory", default=32)
    PARSER.add_argument(
        "--proto_vcpus_count", help="Protocol node vcpus count", default=8
    )
    PARSER.add_argument(
        "--proto_bandwidth", help="Protocol node bandwidth", default=16000
    )
    PARSER.add_argument(
        "--strg_proto_memory", help="Storage protocol node memory", default=32
    )
    PARSER.add_argument(
        "--strg_proto_vcpus_count", help="Storage protocol node vcpus count", default=8
    )
    PARSER.add_argument(
        "--strg_proto_bandwidth", help="Storage protocol node bandwidth", default=16000
    )
    PARSER.add_argument("--enable_afm", help="enable AFM", default="null")
    PARSER.add_argument("--afm_memory", help="AFM node memory", default=32)
    PARSER.add_argument("--afm_vcpus_count", help="AFM node vcpus count", default=8)
    PARSER.add_argument("--afm_bandwidth", help="AFM node bandwidth", default=16000)
    PARSER.add_argument(
    ...
    if (
        len(TF["storage_cluster_instance_private_ips"]) == 0
        and len(TF["compute_cluster_instance_private_ips"]) > 0
    ):
        ...
        computenodegrp = generate_nodeclass_config(
            "computenodegrp",
            ARGUMENTS.comp_memory,
            ARGUMENTS.comp_vcpus_count,
            ARGUMENTS.comp_bandwidth,
        )
        managementnodegrp = generate_nodeclass_config(
            "managementnodegrp",
            ARGUMENTS.mgmt_memory,
            ARGUMENTS.mgmt_vcpus_count,
        ...
        and len(TF["storage_cluster_instance_private_ips"]) > 0
        and len(TF["vpc_availability_zones"]) == 1
    ):
        ...
        storagedescnodegrp = generate_nodeclass_config(
            "storagedescnodegrp",
            ARGUMENTS.strg_desc_memory,
            ARGUMENTS.strg_desc_vcpus_count,
            ARGUMENTS.strg_bandwidth,
        )
        storagenodegrp = generate_nodeclass_config(
            "storagenodegrp",
            ARGUMENTS.strg_memory,
            ARGUMENTS.strg_vcpus_count,
            ARGUMENTS.strg_bandwidth,
        )
        ...
        storageprotocolnodegrp = generate_nodeclass_config(
            ...
            ARGUMENTS.strg_proto_memory,
            ARGUMENTS.strg_proto_vcpus_count,
            ARGUMENTS.strg_proto_bandwidth,
        )
        afmgatewaygrp = generate_nodeclass_config(
            "afmgatewaygrp",
            ARGUMENTS.afm_memory,
            ARGUMENTS.afm_vcpus_count,
            ARGUMENTS.afm_bandwidth,
        )
        ...
        and len(TF["storage_cluster_instance_private_ips"]) > 0
        and len(TF["vpc_availability_zones"]) > 1
        and len(TF["storage_cluster_desc_instance_private_ips"]) > 0
    ):
    ...
    if len(TF["vpc_availability_zones"]) > 1:
        total_node_count = (
            len(TF["compute_cluster_instance_private_ips"])
            + len(TF["storage_cluster_desc_instance_private_ips"])
            + len(TF["storage_cluster_instance_private_ips"])
        )
    else:
        total_node_count = len(TF["compute_cluster_instance_private_ips"]) + len(
    ...
    if (
        ARGUMENTS.using_packer_image == "false"
        ...
        ARGUMENTS.using_packer_image == "true"
        and ARGUMENTS.using_rest_initialization == "true"
    ):
    ...
    if (
        ...
        and ARGUMENTS.scale_encryption_type == "gklm"
    ):
    ...
    if cluster_type in ["storage", "combined"]:
        ...
        scale_storage = initialize_scale_storage_details(
            ...
            disks_list,
            int(ARGUMENTS.default_metadata_replicas),
            int(ARGUMENTS.max_metadata_replicas),
            int(ARGUMENTS.default_data_replicas),
            int(ARGUMENTS.max_data_replicas),
            TF["filesets"],
        )
        scale_protocols = initialize_scale_ces_details(
            TF["smb"],
            TF["nfs"],
            TF["object"],
            TF["export_ip_pool"],
